...
function checkAttachedYellowNeighbors({
  ...
  if (enforceLayout && !attachedYellowInOneLine(yellowSquares, orangeSquares, squareSize, gap)) {
    ...
    startMessageElement.textContent = "Nem megfelelő elrendezés: új forduló indul!";
  ...
  if (!enforceLayout && !attachedYellowInOneLine(yellowSquares, orangeSquares, squareSize, gap)) {
    startMessageElement.textContent = "Nem megengedett: a lerakott sárga betűknek egy sorban vagy oszlopban kell lenniük!";
    startMessageElement.classList.remove("hidden");
  ...
  for (let yellow of yellowPositions) {
    ...
    if (hasMixedDirectionStrict(row, col)) {
      if (!onRoundEnd && startMessageElement) {
        startMessageElement.textContent = "Hibás elrendezés: egy sárga mező nem csatlakozhat vízszintesen ÉS függőlegesen!";
        startMessageElement.classList.remove("hidden");
        setTimeout(() => startMessageElement.classList.add("hidden"), 4000);
      }
      if (onRoundEnd) {
  ...
  if (hasForbiddenRedSequence(positionsInWord)) {
    if (!onRoundEnd && startMessageElement) {
      startMessageElement.textContent = "Nem megengedett: két piros betű egymás után a szóban!";
      startMessageElement.classList.remove("hidden");
      setTimeout(() => startMessageElement.classList.add("hidden"), 4000);
    }
    if (onRoundEnd) {
